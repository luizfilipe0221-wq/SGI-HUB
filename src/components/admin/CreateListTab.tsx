...
export function CreateListTab() {
  ...
  async function criarLista() {
    ...
    setLoading(true);
    try {
      // Montar payload de contatos com distribuição por operador
      const contatosPayload: { contato_id: number; nome_operador: string; ordem: number }[] = [];
      let idx = 0;
      for (let i = 0; i < validOps.length; i++) {
        const count = distribution[i].count;
        const opContatos = selectedContatos.slice(idx, idx + count);
        idx += count;
        opContatos.forEach((c, j) => {
      ...
      const operadoresResult = (result as any).operadores as { nome_operador: string; token_operador: string }[];

      if (operadoresResult) {
        // Manter a ordem dos operadores conforme cadastrado
        for (const op of validOps) {
          const opData = operadoresResult.find((o) => o.nome_operador === op);
          if (opData?.token_operador) {
            generatedLinks.push({
              nome: op,
              link: `${window.location.origin}/lista/${opData.token_operador}`,
            });
          }
        }
      }
      ...
      toast({ title: "Erro ao criar lista", description: msg, variant: "destructive", duration: 7000 });
    } finally {
      setLoading(false);
    }
  }
